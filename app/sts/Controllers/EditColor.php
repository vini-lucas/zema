...
class EditColor
{
    ...
    public function index(int $id)
    {
        ...
        $editUser = new \Sts\Models\StsEditColor(); // -> Instancia a classe para recuperar todos os dados do usuário que serão editados.
        $editUser->searchColor($this->id); // -> Instancia e função passando o ID como parâmetro.
        if ($editUser->getResult()) { // -> Se encontrar algum usuário com o ID do parâmetro então:
            ...
            if (!empty($this->dataForm['SendEditColor'])) { // -> Se o usuário clicar no botão "Editar" então:
                unset($this->dataForm['SendEditColor']); // -> Destrua a posição do botão.
                $valInput = new \Sts\Models\helper\StsValInputField();
                $dataFormInput = [$this->dataForm['color'], $this->dataForm['name']]; // -> Valida se os campos foram preenchidos.
                $valInput->valInputField($dataFormInput);
                if ($valInput->getResult()) {
                            $this->dataForm['modified'] = date("Y-m-d H:i:s");
                            $editUser->exeUpdateColor($this->data['form'][0]['id'], $this->dataForm);
            ...
            header("Location: " . URL . "list-colors/index"); // -> Direciona para a página de listar usuários.
        }
        $this->loadView();
    }
    ...
    public function loadView()
    {
        ...
        $loadView->loadView("app/sts/Views/colors/editColor", $this->data);
    }
}
